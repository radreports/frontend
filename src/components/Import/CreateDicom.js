...
export default class CreateDicom extends Component {
    ...
    createDicom = async () => {
        ...
        try {
            ...
            let response = await fetch(images[0], this.props.OrthancID, this._getTags());
            await Promise.all(images.slice(1)
                .map(image => fetch(image, response.ParentSeries, {})));
            this.setState({
    ...
    async componentDidMount() {
        ...
        let tags = [
            ...(this.props.level === "studies" ?
                    await fetch(this.props.OrthancID).then(response => [
                    ...
                    [
                        ...
                        ...(this.props.level === "patients" ?
                            await fetch(this.props.OrthancID).then(response => (Object.entries(response.MainDicomTags).map(([TagName, Value]) =>
                                ({
                                    TagName,
