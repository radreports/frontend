...
////
    ...
    useEffect(() => {
        const client = new FHIR.client("https://hapi.fhir.org/baseR4");
    ...
    const modalityTemplate = (rowData) =>{
         ...
         try {
            modality = rowData.resource.series[0].modality.display;
         return (
            <>
                <span className="p-column-title">BodyPartExamined</span>
                {modality}
   ...
   const InferResultTemplate = (rowData) =>{
    ...
    <DataTable ref={dt} value={studies} selection={selectedStudies} onSelectionChange={(e) => setSelectedStudies(e.value)}
        ...
        <Column field="bodypart" header="Modality" sortable body={BodypartTemplate} headerStyle={{ width: '34%', minWidth: '10rem' }}></Column>
